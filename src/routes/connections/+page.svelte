...
<PageShell user={data.user}>
  ...
  <div class="max-w-4xl mx-auto space-y-8">
    ...
    <Tabs.Root bind:value={connectionFilter} class="w-full">
      <Tabs.List
        class="grid w-full max-w-md grid-cols-4 bg-[#111] p-1 border border-white/10 rounded-xl mb-6"
      >
        <Tabs.Trigger
          value="connected"
          class="rounded-lg text-xs font-medium data-[state=active]:bg-amber-500/10 data-[state=active]:text-amber-400 data-[state=active]:shadow-sm"
        >
          Connected
        </Tabs.Trigger>
        <Tabs.Trigger
          value="met"
          class="rounded-lg text-xs font-medium data-[state=active]:bg-emerald-500/10 data-[state=active]:text-emerald-400 data-[state=active]:shadow-sm"
        >
          Met
        </Tabs.Trigger>
        <Tabs.Trigger
          value="received"
          class="rounded-lg text-xs font-medium data-[state=active]:bg-cyan-500/10 data-[state=active]:text-cyan-400 data-[state=active]:shadow-sm"
        >
          Received
        </Tabs.Trigger>
        <Tabs.Trigger
          value="sent"
          class="rounded-lg text-xs font-medium data-[state=active]:bg-white/10 data-[state=active]:text-white data-[state=active]:shadow-sm"
        >
          Sent
        </Tabs.Trigger>
      </Tabs.List>
    ...
    {#if loadingConnections}
      <div
        class="flex flex-col items-center justify-center py-24 border border-white/5 bg-[#111]/50 rounded-2xl"
      >
        <div
          class="w-8 h-8 rounded-full border-2 border-amber-500/30 border-t-amber-400 animate-spin mb-4"
        ></div>
        <p class="text-slate-400 text-sm">Loading connections...</p>
      </div>
    {:else if filteredConnections.length === 0}
      <div
        class="flex flex-col items-center justify-center py-24 border border-white/5 bg-[#111]/50 rounded-2xl text-center px-6"
        in:fade
      >
        <div
          class="w-16 h-16 rounded-full bg-white/5 border border-white/10 flex items-center justify-center mb-4 text-slate-500"
        >
          <Users size={24} />
        </div>
        ...
        <Button
          href="/events"
          variant="outline"
          class="border-white/10 bg-[#111] hover:bg-white/5 hover:text-white"
        >
          Events Events
        </Button>
      </div>
    {:else}
      <div class="grid sm:grid-cols-2 gap-4" in:fade>
        {#each filteredConnections as conn (conn.id)}
          <div
            class="group flex flex-col p-5 bg-[#111] border border-white/5 rounded-2xl hover:border-white/10 hover:-translate-y-1 transition-all duration-300"
          >
            <div class="flex justify-between items-start mb-4">
              <div class="flex items-center gap-3">
                <div
                  class="w-10 h-10 rounded-full bg-amber-500/20 text-amber-300 flex items-center justify-center font-bold text-sm shrink-0 border border-amber-500/30 shadow-[0_0_15px_rgba(251,191,36,0.15)]"
                >
              ...
              <a href={`/event/${conn.event_slug}`} class="shrink-0">
                <Badge
                  variant="outline"
                  class="bg-cyan-500/10 text-cyan-400 border-cyan-500/20 hover:bg-cyan-500/20 flex items-center gap-1.5 transition-colors"
                >
                  <MapPin size={10} />
                  <span class="truncate max-w-[80px]" title={conn.event_name}
                    >{conn.event_name}</span
                  >
                </Badge>
              </a>
            </div>
            ...
            <div
              class="mt-auto pt-4 flex items-center gap-2 border-t border-white/5"
            >
              <Button
                variant="outline"
                size="sm"
