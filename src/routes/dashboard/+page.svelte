...
<PageShell user={data.user}>
  ...
  <div class="max-w-5xl mx-auto space-y-8 px-4 sm:px-6">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 animate-slide-up-delay-1">
      <!-- Total Events -->
      <div class="p-5 bg-[#111] border border-white/5 rounded-2xl flex items-center gap-4 hover:border-white/10 transition-colors">
        <div class="w-12 h-12 rounded-full bg-cyan-500/10 text-cyan-400 flex items-center justify-center border border-cyan-500/20">
          <Calendar size={20} />
        </div>
        <div>
      ...
      <div class="p-5 bg-[#111] border border-white/5 rounded-2xl flex items-center gap-4 hover:border-white/10 transition-colors">
        ...
        <div>
          ...
          <p class="text-2xl font-bold text-white">{data.stats.totalConnections}</p>
        </div>
      </div>
    ...
    <div class="animate-slide-up-delay-2 space-y-4">
      <div class="flex items-center justify-between">
        ...
        <Button href="/discover" variant="ghost" class="text-sm text-slate-400 hover:text-white h-8 px-3">
          View all <ArrowRight size={14} class="ml-1.5" />
        </Button>
      </div>

      {#if data.recentEvents.length === 0}
        <div class="flex flex-col items-center justify-center py-16 border border-white/5 bg-[#111]/50 rounded-2xl text-center px-6">
          ...
          <Button href="/discover" variant="outline" class="border-white/10 bg-[#111] hover:bg-white/5 text-white">
            Discover Events
          </Button>
        </div>
      {:else}
        <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {#each data.recentEvents as event}
            <div class="flex flex-col p-5 bg-[#111] border border-white/5 rounded-2xl hover:border-white/10 transition-colors h-full">
              <div class="flex justify-between items-start mb-3">
                <Badge variant="outline" class={event.role === 'organizer' ? 'bg-amber-500/10 text-amber-400 border-amber-500/20' : 'bg-cyan-500/10 text-cyan-400 border-cyan-500/20'}>
              ...
              <h3 class="text-lg font-bold text-white mb-2 line-clamp-1">{event.name}</h3>
